...
def _check_feature_enabled(feature_name,log_path=None):
    ...
    try:
        ...
        WriteFile(log_path,f"检查{feature_name}返回内容stdout：{result.stdout}",mode='a',write=True)
        WriteFile(log_path,f"检查{feature_name}返回内容stderr：{result.stderr}",mode='a',write=True)
        # 确保输出不为None
        output = result.stdout or ""
        return "State : Enabled" in output or "状态 : 已启用" in output
    except Exception as e:
        WriteFile(log_path,f"检查功能状态失败: {str(e)}",mode='a',write=True)
        return False
    ...
    try:
        ...
        for feature in features:
            if not _check_feature_enabled(feature,log_path=log_path):
                WriteFile(log_path,f"正在启用功能: {feature}",mode='a',write=True)
                sout,serr,retcode = RunCommand(f"dism /online /enable-feature /featurename:{feature} /all /NoRestart",returncode=True)
                if retcode == 3010:
                    WriteFile(log_path,f"已安装{feature}，但需要重启windows生效！！！",mode='a',write=True)
                elif retcode == 0:
                    WriteFile(log_path,f"安装{feature}，成功",mode='a',write=True)
                else:
        ...
        # )
        WriteFile(log_path,"更新WSL内核",mode='a',write=True)
        ...
        if ok:
            try:
                ...
                WriteFile(log_path,f"更新WSL内核失败：{e}",mode='a',write=True)
        #删除下载的文件
        DeleteFile(save_path,empty_tips=False)
        
        # 设置默认版本为WSL2
        WriteFile(log_path,"正在设置WSL2为默认版本...",mode='a',write=True)
        subprocess.run(
        ...
        WriteFile(log_path,"正在安装linux发行版Centos7...",mode='a',write=True)
        #安装linux发行版
        ...
        WriteFile(log_path,f"安装过程中出错: {e}",mode='a',write=True)
        return False
    ...
    try:
        ...
        if is_windows:
            ...
            if not install_env_res:
                WriteFile(log_path,"安装Windows虚拟化环境失败，请手动安装！！！",mode='a',write=is_write_log)
            ...
            #docker-compose
            compose_download_url = "https://github.com/docker/compose/releases/download/v2.33.0/docker-compose-windows-x86_64.exe"
...
def Uninstall_Docker(is_windows=True):
    ...
    if is_windows:
        if os.path.exists(install_path):
            ...
            time.sleep(0.1)
            try:
                exe_path = soft_paths['windows_abspath_dockerd_bin']
                subprocess.run([exe_path,'--unregister-service','--service-name=dockerd'], capture_output=False, text=True)
                # subprocess.run(['sc','delete','dockerd'], capture_output=False, text=True)
            except Exception as e:
                print(f"卸载dokcerd service 失败：{e}")
...
def is_docker_running(is_windows=True,simple_check=False):
    ...
    soft_name ='dockerd.exe' if is_windows else "dockerd"
        ...
        if os.path.exists(exe_path):
            try:
                if not is_docker_running(is_windows=True):
                    subprocess.run(['net','start','dockerd'], capture_output=False, text=True)
                else:
                    r_status = True
                    return True
                time.sleep(1.5)
    ...
    if is_windows:
        ...
        try:
            if is_docker_running(is_windows=True):
                code = RunCommandReturnCode(['net',"stop", "dockerd"])
