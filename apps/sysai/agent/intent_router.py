...
_DEPLOY_KEYWORDS = {'帮我部署', '部署wordpress', '部署nextcloud', '部署gitlab', '部署mysql', '部署redis', '部署mongodb', '部署postgresql', '部署应用', '一键部署', '部署项目', '部署网站', '部署django', '部署flask', '部署fastapi', '部署express', '部署nextjs', '部署vue', '部署react', '部署go项目', '部署php项目', 'git项目部署', '部署git', '部署github', '部署gitee', '部署压缩包', '解压部署'}

_DEPLOY_GIT_PATTERN = r'(?:https?://|git@|ssh://git@)(?:gitee|github|gitlab)[\w./:@\-]+(?:\.git)?'

_DEPLOY_PATH_PATTERN = r'(?:/www/|/root/|/home/|/var/www/|/opt/|D:/|C:/)[\w./\-]+'

_DEPLOY_ARCHIVE_PATTERN = r'(?:/www/|/root/|/home/|/var/www/|/opt/|D:/|C:/)[\w./\-]+\.(?:zip|tar\.gz|tar\.bz2|tgz|rar|7z)'


def _is_deploy_intent(user_input: str) -> bool:
    """检测是否为部署意图"""
    import re
    input_lower = user_input.lower()
    # 关键词匹配
    if any(kw in input_lower for kw in _DEPLOY_KEYWORDS):
        return True
    # Git仓库地址匹配（支持带和不带.git后缀）
    if re.search(_DEPLOY_GIT_PATTERN, user_input, re.IGNORECASE):
        return True
    # 压缩包路径匹配
    if re.search(_DEPLOY_ARCHIVE_PATTERN, user_input, re.IGNORECASE):
        return True
    # 本地路径 + 部署关键词
    if re.search(_DEPLOY_PATH_PATTERN, user_input, re.IGNORECASE) and any(kw in input_lower for kw in ['部署', '发布', '上线', '运行', '启动']):
        return True
    return False
...
def resolve_tools(
    ...
    # 部署意图：加载panel_deploy工具集 + 绑定deploy_app Agent
    if _is_deploy_intent(user_input):
        deploy_toolsets = ['panel_deploy']
        tool_names = resolve_tools_by_toolsets(deploy_toolsets)
        tool_names.extend(t for t in TASK_TOOLS if t not in tool_names)
        tool_names.extend(t for t in DOC_TOOLS if t not in tool_names)
        # 获取deploy_agent的system_prompt
        agent_prompt = None
        try:
            from apps.sysai.agent.specialized import get_agent_class
            agent_cls = get_agent_class('deploy_app')
            if agent_cls:
                agent_prompt = agent_cls().system_prompt
        except Exception:
            pass
        logger.info(f'意图路由: 检测到部署意图, 加载panel_deploy工具集+deploy_app Agent({len(tool_names)}个工具)')
        return tool_names, agent_prompt
