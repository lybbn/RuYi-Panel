...
class Agent:
    ...
    def confirm_tool(self, confirm_id: str, approved: bool):
        with self._confirm_lock:
    ...
    def _execute_tool(self, tool_name: str, arguments: Dict[str, Any]) -> str:
        ...
        try:
            ...
            is_success = '<toolcall_status>done</toolcall_status>' in result
            skill_evolution.record_tool_usage(tool_name, is_success)
        except Exception:
            pass

        try:
            from apps.sysai.agent.skill_evolution import session_tracker
            is_success = '<toolcall_status>done</toolcall_status>' in result
            session_tracker.record_tool_call(tool_name, arguments, is_success)
        except Exception:
            pass
    ...
    def chat(
        ...
        try:
            ...
            try:
                from apps.sysai.agent.skill_evolution import session_tracker
                session_tracker.start_session(user_input, self.session_id)
            except Exception:
                pass
            ...
            try:
                from apps.sysai.agent.skill_evolution import session_tracker, skill_evolution
                session_data = session_tracker.end_session()
                if session_data:
                    skill_evolution.record_session_summary(session_data)
            except Exception:
                pass
