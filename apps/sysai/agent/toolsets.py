...
TOOLSETS: Dict[str, Dict[str, Any]] = {
    ...
    'panel_website': {
        'name': '面板网站管理',
        'description': '如意面板网站站点管理、项目部署、反向代理、SSL证书',
        'trigger_keywords': ['面板网站', '创建网站', '站点管理', '添加网站', '绑定域名', '网站列表', '部署项目', '部署站点', '部署网站', '反向代理', 'ssl证书', 'https', '部署python', '部署node', '部署go', '部署php', '部署django', '部署flask', '部署fastapi', '部署express', '部署nextjs', 'git项目部署'],
        'tools': ['panel_site_list', 'panel_site_create',
                  'panel_site_manage', 'panel_site_domains',
                  'panel_runtime_site_create', 'panel_site_ssl',
                  'panel_site_proxy', 'panel_deploy_project'],
    },
    'panel_database': {
        'name': '面板数据库管理',
...
def get_profile_tools(profile_name: str) -> List[str]:
    ...
    for ts_name in profile.get('toolsets', []):
        ...
        if ts_name in _PANEL_TOOLSETS:
            has_panel_toolset = True
    # 面板工具集自动关联文档工具
    if has_panel_toolset:
...
def _is_panel_general_question(user_input: str) -> bool:
    """检测是否为面板通用问题（如"如意面板如何升级"）"""
    input_lower = user_input.lower()
    # 至少匹配2个通用关键词才认为是面板问题
    match_count = sum(1 for kw in _PANEL_GENERAL_KEYWORDS if kw in input_lower)
...
- "帮我部署LNMP环境" → panel_website,panel_docker,panel_database,task
- "帮我部署一个wordpress" → panel_docker,task
- "容器广场安装mysql" → panel_docker
- "部署一个Django项目" → panel_website,task
- "部署https://gitee.com/xxx.git 站点" → panel_website,task
- "帮我部署一个Python项目" → panel_website,task
