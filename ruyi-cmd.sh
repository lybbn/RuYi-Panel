...
repair() {
    read -p "将从服务器重新下载并修复如意面板，是否继续 [y/n] : " ynstr
    if [ "$ynstr" == "Y" ] || [ "$ynstr" == "y" ]; then
        echo -e "================== 开始修复 RuYi 服务器面板 =================="
        echo -e ""
        
        cp /ruyi/server/ruyi/utils/scripts/update_fix.sh /tmp/update_fix.sh
        cp /ruyi/server/ruyi/utils/scripts/update_init.sh /tmp/update_init.sh
        if [ $? -eq 0 ]; then
            echo "开始执行修复..."
        else
            echo "未发现面板修复脚本，请联系官网获取！！！"
            exit 0;
        fi
        ...
        if [ $? -eq 0 ]; then
            ...
            rm -rf /tmp/update_fix.sh
            rm -rf /tmp/update_init.sh
            rm -rf /tmp/ruyi
            systemctl restart ruyi
            exit 0
        else
            echo "执行修复失败！！！"
        fi
